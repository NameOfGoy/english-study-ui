  ...
  <div class="review-page">
    ...
    <div v-if="finishedAll" class="completion-overlay">
      <div class="completion-card">
        ...
        <h2 class="completion-title">{{ finishReason === 'empty' ? '当前已没有需要复习的单词' : '本次复习完成！' }}</h2>
        <p class="completion-sub">
          <span v-if="finishReason === 'completed'">🎉 恭喜你，坚持完成了本次复习</span>
          <span v-else>👏 当前任务已清空，可稍后再来或切换模式</span>
        </p>
        <div class="completion-stats" v-if="finishReason === 'completed'">
          本次共复习 <span class="number">{{ wordCards.length }}</span> 个单词
        </div>
        ...
        <div class="completion-actions">
          ...
          <van-button type="success" round @click="reloadReviewList">再来一组</van-button>
        </div>
      </div>
    ...
    >
      ...
      <div v-if="reviewState === 'initial'" class="review-initial">
        ...
        <div v-if="hasImages" class="section picture-section">
          ...
          <div class="picture-carousel">
            ...
            >
              <!-- 当检测到纵向手势意图时，仅在轮播内部启用遮罩，避免遮罩影响下方按钮 -->
      ...
      <div v-if="reviewState === 'full'" class="review-full">
        ...
        <!-- 完整状态按钮 -->
        <div class="section actions-section">
          <van-button type="success" block :loading="finishing" @click="onFinishReview(1)">下一个</van-button>
          <van-button type="danger" block :loading="finishing" @click="onFinishReview(2)">不记得</van-button>
        </div>
      </div>
    </div>
  </div>
...
import { getReviewWordCardList, finishReview, PRACTISE_OPERATION } from '@/api/practise'
...
export default {
  ...
  setup() {
    ...
    // 复习状态管理
    const reviewState = ref('initial') // 'initial' | 'hint' | 'full'
    ...
    const loadWordCards = async () => {
      try {
        ...
        const response = await getReviewWordCardList({ count: 10, random: true })
    ...
    // 完成复习
    const onFinishReview = async (operation) => {
      if (!currentCard.value) return
      try {
        ...
        if (safeWordType !== rawWordType) {
          console.warn('复习卡片的 word_type 非法，已回退为 1（单词）：', rawWordType, currentCard.value)
        }
        const payload = {
        ...
        await finishReview(payload)
        
        const message = operation === 1 ? '已完成复习' : '已标记为不记得'
        ...
        console.error('完成复习失败:', e)
        showToast({ message: '完成复习失败', type: 'fail' })
      } finally {
...
.ctrl-btn { 
  ...
  align-items: center;
  justify-content: center;
  padding: 0;           /* 去除按钮默认内边距，避免偏移 */
  line-height: 1;       /* 消除字体行高影响，保证垂直居中 */
  text-align: center;   /* 文本水平居中（与 flex 对齐互补） */
