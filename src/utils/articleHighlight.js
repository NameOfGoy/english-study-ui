// 文章高亮分词: 把一句英文按"目标词的字面形式(surfaces)"切成普通段与高亮段.
// 设计要点:
...
//  - 输出 segment 数组, 由 Vue 用 <span> 渲染(文本自动转义, 无 XSS, 无需 v-html/DOMPurify)
...
/**
 * @param {string} text 一句英文
 * @param {Array<{word:string,type:number,surfaces?:string[]}>} marks 目标词
 * @returns {Array<{text:string, word:string|null, type:number|null}>} 段数组; word 非空即高亮段
 */
export function buildSegments(text, marks) {
  ...
  for (const m of marks || []) {
    ...
    const list = m && m.surfaces && m.surfaces.length ? m.surfaces : [m && m.word]
    for (const sf of list) {
      const s = (sf || '').trim()
      if (s) surfaces.push({ surface: s, word: m.word, type: mtype })
    }
  }
  ...
  // 最长优先, 再按起点; 同位置时"词与命中文本完全一致"者优先(防被错配的同形 surface 抢占); 然后贪心占位
  matches.sort((a, b) => {
    const byLen = b.end - b.start - (a.end - a.start)
    if (byLen !== 0) return byLen
    if (a.start !== b.start) return a.start - b.start
    const aExact = a.word && text.slice(a.start, a.end).toLowerCase() === String(a.word).toLowerCase()
    const bExact = b.word && text.slice(b.start, b.end).toLowerCase() === String(b.word).toLowerCase()
  ...
  for (const m of accepted) {
    if (m.start > cur) segs.push({ text: text.slice(cur, m.start), word: null, type: null })
    segs.push({ text: text.slice(m.start, m.end), word: m.word, type: m.type })
    cur = m.end
  }
  if (cur < text.length) segs.push({ text: text.slice(cur), word: null, type: null })
  return segs
}

// key 用于按 (word, type) 在 used_words 里查气泡卡信息
export function wordKey(word, type) {
